...
import {
  buildEmptyTeamExecutionSummaryReadModel,
  getTeamExecutionSummaryReadModel,
...
export const GET = withAuth(
  async (request: NextRequest, { userId }) => {
    try {
      ...
      const readModel = await getTeamExecutionSummaryReadModel(userId, filters, dateRange);

      return successResponse(readModel);
    } catch (error) {
      console.error('Get team execution summary error:', error);
      const filters = parseTeamExecutionFilters(request.nextUrl.searchParams);
      return successResponse(buildEmptyTeamExecutionSummaryReadModel(filters));
    }
  },
  {
