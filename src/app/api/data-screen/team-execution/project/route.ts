...
import {
  buildEmptyTeamExecutionProjectReadModel,
  getTeamExecutionProjectReadModel,
...
export const GET = withAuth(
  async (request: NextRequest, { userId }) => {
    try {
      ...
      const readModel = await getTeamExecutionProjectReadModel(userId, filters, dateRange);

      return successResponse(readModel);
    } catch (error) {
      console.error('Get team execution project view error:', error);
      const filters = parseTeamExecutionFilters(request.nextUrl.searchParams);
      return successResponse(buildEmptyTeamExecutionProjectReadModel(filters));
    }
  },
  {
