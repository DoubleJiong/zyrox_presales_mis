...
import {
  buildEmptyTeamExecutionSolutionReadModel,
  getTeamExecutionSolutionReadModel,
...
export const GET = withAuth(
  async (request: NextRequest, { userId }) => {
    try {
      ...
      const readModel = await getTeamExecutionSolutionReadModel(userId, filters, dateRange);

      return successResponse(readModel);
    } catch (error) {
      console.error('Get team execution solution view error:', error);
      const filters = parseTeamExecutionFilters(request.nextUrl.searchParams);
      return successResponse(buildEmptyTeamExecutionSolutionReadModel(filters));
    }
  },
  {
